...
function PlainDiseasePanel({ details }: { details: DiseaseDetail }) {
  return (
    <div className="space-y-4">
      ...
      {details.symptoms.length > 0 && details.precautions.length > 0 && <div className="border-t border-white/10" />}
...
export default function SafetyPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/public/disease-details')
        ...
        .then((data: unknown) => (Array.isArray(data) ? (data as DiseaseDetail[]) : []))
        .catch(() => [] as DiseaseDetail[]),
      fetch('/api/public/diseases')
        .then((response) => response.json())
        .then((data: unknown) => {
          if (Array.isArray(data)) return data as Disease[];
          const payload = data as { diseases?: Disease[] };
          return payload?.diseases ?? [];
        })
        .catch(() => [] as Disease[]),
    ])
      .then(([detailsList, diseasesList]) => {
  ...
  const merged = useMemo<Merged[]>(() => {
    const detailMap = new Map(details.map((detail) => [detail.disease_id, detail]));
    const diseaseMap = new Map(diseases.map((disease) => [disease.disease_id, disease]));
    const allIds = new Set([...details.map((detail) => detail.disease_id), ...diseases.map((disease) => disease.disease_id)]);
    ...
    return Array.from(allIds)
      ...
      .sort((left, right) => {
        if (left.details && !right.details) return -1;
        if (!left.details && right.details) return 1;
        return left.disease_name.localeCompare(right.disease_name);
      });
  }, [diseases, details]);

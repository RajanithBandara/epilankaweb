...
export async function DELETE(req: NextRequest) {
    try {
        ...
        if (!reportid || !location) {
            return NextResponse.json({ error: "Missing reportid or location" }, { status: 400 });
        }
        ...
        const response = await axios.delete(
            `${fastApiUrl}/user_reports/delete?reportid=${encodeURIComponent(reportid)}&location=${encodeURIComponent(location)}`,
            {
                headers: {
                    "x-api-key": apiKey || "",

...
export async function DELETE(req: NextRequest) {
    try {
        ...
        const cookieToken = req.cookies.get("appwrite-admin-jwt")?.value;
        ...
        const response = await axios.delete(
            `${fastApiUrl}/admin/user-reports/${encodeURIComponent(report_id)}?district=${encodeURIComponent(district)}`,
            {
                headers: {
                    "x-api-key": apiKey || "",

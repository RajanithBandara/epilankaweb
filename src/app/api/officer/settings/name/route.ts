...
export async function PUT(req: NextRequest) {
  try {
    ...
    const response = await api.put("/officer/settings/name", body);
    return NextResponse.json(response.data, { status: 200 });
  } catch (error: unknown) {
    const err = error as { response?: { data?: { detail?: string; error?: string }; status?: number }; message?: string };
    const message = err.response?.data?.detail || err.response?.data?.error || err.message || "Failed to update name";

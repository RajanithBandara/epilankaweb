...
export default function NotificationManage() {
    ...
    const fetchNotifications = useCallback(async () => {
        try {
            setListLoading(true);
            const response = await fetch("/api/admin/notifications?skip=0&limit=50", { cache: "no-store" });
            const data = (await response.json()) as { items?: AdminNotification[]; error?: string };

            if (!response.ok) {
                throw new Error(data.error || "Failed to load notifications");
            }

            // Sometimes the API returns the array directly, so handle both
        ...
        try {
            const payload = {
                ...
                metadata: {
                    ...
                    created_from: "admindashboard/notifications",
                },
            };
            ...
            const data = (await response.json()) as { error?: string; message?: string; notification?: { notification_id?: string }; notification_id?: string };
            if (!response.ok) {
                throw new Error(data.error || "Failed to push notification");
            }

            const mode = payload.user_id ? `user ${payload.user_id}` : "all users";
            const id = data.notification?.notification_id || data.notification_id ? ` (${data.notification?.notification_id || data.notification_id})` : "";
            setSuccess(`Notification pushed to ${mode}${id}.`);

            const created = data.notification as AdminNotification | undefined;
            ...
            <div className="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p className="mt-2 text-sm text-black/60 dark:text-white/60">
                        Send live notifications to all users or a specific user from the admin dashboard.
                    </p>
                </div>
            ...
            <Card className="border-black/10 dark:border-white/15">
                <CardHeader className="flex flex-row items-center justify-between gap-3">
                    <div>
                        ...
                        <CardDescription>
                            Admins can view, edit, and delete notifications from this list.
                        </CardDescription>
                    </div>
                    <Button
                        type="button"
                            ...
                            {notifications.map((notification) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className="rounded-xl border border-black/10 bg-gradient-to-br from-white to-black/5 p-4 shadow-sm dark:border-white/15 dark:from-black dark:to-white/5"
                                    >
